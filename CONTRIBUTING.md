# Contributing to ep_show_whitespace

Thanks for helping improve `ep_show_whitespace` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_show_whitespace` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_show_whitespace from your local clone
pnpm run plugins i --path ../ep_show_whitespace

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_show_whitespace/` are picked up after a server restart.
...
* Etherpad version, ep_show_whitespace version, browser + OS
* Steps to reproduce
